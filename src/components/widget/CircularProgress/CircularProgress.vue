        ...
        methods: {
            draw(){
                ...
                this.text = new Konva.Text({
                    x: this.currentX,
                    y: this.currentX,
                    text: Math.ceil(this.msg * 100 / (this.maxValue - this.minValue)) + "%",
                ...
                let angle = this.msg * 360 / (this.maxValue - this.minValue);
                this.arc.to({
                    angle: angle,
                    duration: .8,
                    ...
                    setTimeout(() => {
                        ...
                        this.timer = setInterval(() => {
                            ...
                            if (currentValue > oldValue && currentValue < current) {
                                ...
                                if (current < 10) {
                                    this.text.text(" " + Math.floor(current * 100 / (this.maxValue - this.minValue)) + "%");
                                } else {
                                    this.text.text(Math.floor(current * 100 / (this.maxValue - this.minValue)) + "%");
                                }
                            }
                        ...
                        this.arc.to({
                            ...
                            onFinish: () => {
                                ...
                                if (this.timer)
                                    clearInterval(this.timer);
                                if (currentValue < 10) {
                                    this.text.text(" " + Math.floor(currentValue * 100 / (this.maxValue - this.minValue)) + "%");
                                } else {
                                    this.text.text(Math.floor(currentValue * 100 / (this.maxValue - this.minValue)) + "%");
                                }
                            }
