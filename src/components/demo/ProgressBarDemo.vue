<template>
    <div class="search-bar-demo">
        <appbar :vtitle="'ProgressBar'" @appbarLeftClick="handleLeft" :haveMenu="false"></appbar>
        <button @click="showPregressBar">显示PregressBar</button>
        <progressbar ref="pregressbar"></progressbar>
        <div class="tips-container">
            <div>使用说明</div>
            <p>
                demo：src/components/demo/ProgressBarDemo.vue
            </p>
        </div>
    </div>
</template>

<script type="text/ecmascript-6">
    import appbar from "components/widget/AppBar/AppBar.vue"
    import progressbar from "components/widget/ProgressBar/ProgressBar.vue"
    export default {
        created() {
            this.$router.isBack = false;
        },
        data() {
            return {

            }
        },
        components: {
            appbar,
            progressbar
        },
        methods: {
            handleLeft(){
                this.$router.go(-1)
            },
            showPregressBar(){
                this.$refs.pregressbar.show("提交中...");
                let that = this;
                setTimeout(function () {
                    that.$refs.pregressbar.hide();
                }, 8000)
            }
        }
    }
</script>

<style lang="stylus" rel="stylesheet/stylus" type="text/stylus" scoped>
    .search-bar-demo
        background: #f7f7f7
        button
            margin-top 16px
            margin-left 16px
        .tips-container
            padding 16px
            & > div
                font-size 16px
                font-weight bold
                line-height 36px
            & > p
                line-height 21px

</style>